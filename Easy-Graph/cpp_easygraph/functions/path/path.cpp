...
py::object _invoke_cpp_dijkstra_multisource(py::object G,py::object sources, py::object weight, py::object target) {
    ...
    for(int i = 0; i < sources_list_len; i++){
        if(G_.node_to_id.attr("get")(sources_list[i],py::none()).is(py::none())){
            py::pybind11_fail("source node should exist in the graph");
        }
...
py::object _invoke_gpu_dijkstra_multisource(py::object G,py::object py_sources, py::object weight, py::object target) {
    ...
    int target_id = -1;
    if (!target.is_none()) {
        if (G_.node_to_id.attr("get")(target, py::none()).is(py::none())) {
            py::pybind11_fail("target node should exist in the graph");
        }
        target_id = G_.node_to_id.attr("get")(target).cast<int>();
    }
    int gpu_r;
    if (weight.is_none()) {
        gpu_r = gpu_easygraph::sssp_unweighted_bfs(V, E, *sources,
                                                  target_id,
                                                  sssp,
                                                  &kernel_seconds);
    } else {
        gpu_r = gpu_easygraph::sssp_dijkstra(V, E, *W_p, *sources,
                                            target_id,
                                            sssp,
                                            &kernel_seconds);
    }
    ...
    py::list source_list = py::list(py_sources);
    const std::vector<node_t>& nodes = csr_graph->nodes;
    const int source_count = std::min((int)source_list.size(), (int)sources->size());
    const int n = std::min((int)nodes.size(), (int)V.size() - 1);
    py::dict values;
    for (int row = 0; row < source_count; ++row) {
        py::dict dist;
        const int base = row * ((int)V.size() - 1);
        for (int i = 0; i < n; ++i) {
            double d = sssp[base + i];
            if (std::isfinite(d) && std::fabs(d) < 1.0e30) {
                node_t internal_id = nodes[i];
                dist[G_.id_to_node[py::cast(internal_id)]] = py::float_(d);
            }
        }
        values[source_list[row]] = dist;
    }
    py::dict out;
    out["values"] = values;
...
py::object _invoke_gpu_dijkstra_multisource_dense(py::object G, py::object py_sources, py::object weight, py::object target) {
    Graph& G_ = G.cast<Graph&>();
    if (weight.is_none()) {
        G_.gen_CSR();
    } else {
    ...
    const int source_count = (int)sources->size();
    const int n = std::max(0, (int)V.size() - 1);
    py::array::ShapeContainer ret_shape{source_count, n};
    py::array_t<double> ret(ret_shape, sssp.data());
    py::dict out;
...
#ifdef EASYGRAPH_ENABLE_GPU
    const char* enable_gpu = std::getenv("EASYGRAPH_ENABLE_GPU");
    if (enable_gpu != nullptr) {
        std::string flag(enable_gpu);
        if (flag == "1" || flag == "TRUE" || flag == "true" || flag == "ON" || flag == "on" || flag == "YES" || flag == "yes") {
...
py::object _dijkstra_multisource_gpu_dense(py::object G, py::object sources, py::object weight, py::object target) {
#ifdef EASYGRAPH_ENABLE_GPU
    return _invoke_gpu_dijkstra_multisource_dense(G, sources, weight, target);
#else
    py::dict out;
    out["values_dense"] = _invoke_cpp_dijkstra_multisource(G, sources, weight, target);
    out["kernel_seconds"] = py::float_(0.0);
    return out;
#endif
}

py::object _bellman_ford_multisource_gpu_dense(py::object G, py::object py_sources, py::object weight, py::object target) {
#ifdef EASYGRAPH_ENABLE_GPU
    Graph& G_ = G.cast<Graph&>();
    if (weight.is_none()) {
        G_.gen_CSR();
    ...
    int gpu_r = gpu_easygraph::sssp_bellman_ford(
        V, E, *W_p, *sources, target_id, sssp, &kernel_seconds);
    if (gpu_r != gpu_easygraph::EG_GPU_SUCC) {
        py::pybind11_fail(gpu_easygraph::err_code_detail(gpu_r));
    }
    ...
    return out;
#else
    py::pybind11_fail("EasyGraph GPU Bellman-Ford backend is not built");
#endif
}
