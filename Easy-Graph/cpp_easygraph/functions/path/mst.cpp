...
py::object _mst_gpu_native(py::object G, py::object weight) {
    ...
    for (int i = 0; i < limit; ++i) {
        int su = mst_src[i];
        int sv = mst_dst[i];
        if (su < 0 || sv < 0 || su >= (int)nodes.size() || sv >= (int)nodes.size()) continue;
        node_t id_u = nodes[su];
        node_t id_v = nodes[sv];
        edges.append(py::make_tuple(
            G_.id_to_node[py::cast(id_u)],
            G_.id_to_node[py::cast(id_v)],
            py::float_(mst_weight[i])
        ));
    }
    py::dict out;
    out["edges"] = edges;
...
py::object _mst_gpu_native_index_edges(py::object G, py::object weight) {
#ifdef EASYGRAPH_ENABLE_GPU
    Graph& G_ = G.cast<Graph&>();
    std::string weight_key = weight_to_string(weight);
    if (weight.is_none()) {
    ...
    const int limit = std::min({(int)mst_src.size(), (int)mst_dst.size(), (int)mst_weight.size()});
    for (int i = 0; i < limit; ++i) {
        index_edges.append(py::make_tuple(
            py::int_(mst_src[i]),
            py::int_(mst_dst[i]),
            py::float_(mst_weight[i])
        ));
    }
    out["index_edges"] = index_edges;
    out["kernel_seconds"] = py::float_(kernel_seconds);
    return out;
#else
    py::dict out;
    out["index_edges"] = py::list();
    out["kernel_seconds"] = py::float_(0.0);
    return out;
#endif
}

py::object _mst_gpu_native_tree(py::object G, py::object weight) {
#ifdef EASYGRAPH_ENABLE_GPU
    Graph& G_ = G.cast<Graph&>();
    std::string weight_key = weight_to_string(weight);
    if (weight.is_none()) {
    ...
    const int limit = std::min({(int)mst_src.size(), (int)mst_dst.size(), (int)mst_weight.size()});
    py::array_t<int> src_arr(limit);
    py::array_t<int> dst_arr(limit);
    py::array_t<double> weight_arr(limit);
    int* src_data = src_arr.mutable_data();
    int* dst_data = dst_arr.mutable_data();
    double* weight_data = weight_arr.mutable_data();
    for (int i = 0; i < limit; ++i) {
        src_data[i] = mst_src[i];
        dst_data[i] = mst_dst[i];
        weight_data[i] = mst_weight[i];
    }

    tree.attr("_node") = node_dict;
    tree.attr("_adj") = py::dict();
    tree.attr("_node_index") = node_index;
    tree.attr("_id") = py::int_(n);
    py::dict cache;
    cache["_lazy_edge_arrays"] = py::make_tuple(src_arr, dst_arr, weight_arr, weight);
    tree.attr("cache") = cache;
