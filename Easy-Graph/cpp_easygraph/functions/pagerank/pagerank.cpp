...
py::object _pagerank_gpu_native(py::object G, double alpha, int max_iterator, double threshold, py::object weight) {
    ...
    int gpu_r = gpu_easygraph::pagerank(
        V,
        E,
        *W_p,
        alpha,
    ...
    const int n = std::min((int)nodes.size(), (int)pr_values.size());
    for (int i = 0; i < n; ++i) {
        node_t internal_id = nodes[i];
        values[G_.id_to_node[py::cast(internal_id)]] = py::float_(pr_values[i]);
    }
    result["values"] = values;
    result["kernel_seconds"] = py::float_(kernel_seconds);
    return result;
#else
    py::dict result;
    result["values"] = _pagerank(G, alpha, max_iterator, threshold, weight);
    result["kernel_seconds"] = py::float_(0.0);
    return result;
#endif
}
...
py::object _pagerank_gpu_native_dense(py::object G, double alpha, int max_iterator, double threshold, py::object weight) {
    ...
    py::dict result;
    py::array_t<double> values_dense(pr_values.size());
    if (!pr_values.empty()) {
        std::copy(pr_values.begin(), pr_values.end(), values_dense.mutable_data());
    }
    result["values_dense"] = values_dense;
    result["kernel_seconds"] = py::float_(kernel_seconds);
    return result;
#else
    py::dict result;
    result["values_dense"] = py::list();
    result["kernel_seconds"] = py::float_(0.0);
    return result;
#endif
}
