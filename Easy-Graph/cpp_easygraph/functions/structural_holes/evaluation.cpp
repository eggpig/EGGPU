...
static py::object invoke_gpu_efficiency(py::object G, py::object nodes, py::object weight) {
    ...
    for (auto item : effective_size_dict) {
        py::object node = py::reinterpret_borrow<py::object>(item.first);
        double eff_size = py::reinterpret_borrow<py::object>(item.second).cast<double>();

        if (!degree.contains(node)) {
            continue;
        }

        double node_degree = py::reinterpret_borrow<py::object>(degree[node]).cast<double>();
        if (node_degree == 0.0) {
            efficiency_dict[node] = py::cast(Py_NAN);
        } else {
            double efficiency_value = eff_size / node_degree;
            efficiency_dict[node] = py::cast(efficiency_value);
        }
    }
...
static py::dict invoke_gpu_structural_dense(
    ...
    py::array::ShapeContainer ret_shape{(int)results.size()};
    py::dict out;
    out["values_dense"] = py::array_t<double>(ret_shape, results.data());
    out["kernel_seconds"] = kernel_seconds;
    return out;
}

py::object gpu_constraint_dense(py::object G, py::object nodes, py::object weight) {
    return invoke_gpu_structural_dense(G, nodes, weight, gpu_easygraph::constraint);
}

py::object gpu_effective_size_dense(py::object G, py::object nodes, py::object weight) {
    return invoke_gpu_structural_dense(G, nodes, weight, gpu_easygraph::effective_size);
}

py::object gpu_hierarchy_dense(py::object G, py::object nodes, py::object weight) {
    return invoke_gpu_structural_dense(G, nodes, weight, gpu_easygraph::hierarchy);
}
#endif
