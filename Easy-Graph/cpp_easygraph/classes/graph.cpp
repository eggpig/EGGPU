...
std::shared_ptr<CSRGraph> Graph::gen_CSR(const std::string& weight) {
    if (csr_graph != nullptr) {
        ...
        if (csr_graph->W_map.find(weight) == csr_graph->W_map.end()) {
            ...
            // CSR rows are stored sorted by destination index.  Build the
            // weight vector in exactly the same order instead of relying on
            // unordered_map iteration order.
            for (int idx = 0; idx < static_cast<int>(csr_graph->nodes.size()); ++idx) {
                node_t n = csr_graph->nodes[idx];
                // if n is not in adj, this way can raise an exception
                const auto& n_adjs = adj.find(n)->second;

                for (int edge_pos = csr_graph->V[idx]; edge_pos < csr_graph->V[idx + 1]; ++edge_pos) {
                    node_t dst = csr_graph->nodes[csr_graph->E[edge_pos]];
                    auto adj_it = n_adjs.find(dst);
                    if (adj_it == n_adjs.end()) {
                        W->push_back(1.0);
                        continue;
                    }
        ...
        for (int idx = 0; idx < nodes.size(); ++idx) {
            ...
            std::vector<std::pair<int, weight_t>> row_entries;
            row_entries.reserve(n_adjs.size());
            for (auto adj_it = n_adjs.begin(); adj_it != n_adjs.end(); ++adj_it) {
                const edge_attr_dict_factory& edge_attr = adj_it->second;
                auto edge_it = edge_attr.find(weight);
                weight_t w = edge_it != edge_attr.end() ? edge_it->second : 1.0;

                row_entries.emplace_back(node2idx[adj_it->first], w);
            }
