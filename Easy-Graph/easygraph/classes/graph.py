...
class Graph:
    ...
    **Attributes:**
    ...
    def _materialize_lazy_edges(self):
        ...
        lazy_edges = self.cache.get("_lazy_edges")
        if lazy_edges is None:
            return
        adj = self.adjlist_outer_dict_factory()
        for node in self._node:
            adj[node] = self.adjlist_inner_dict_factory()
        for edge in lazy_edges:
            if len(edge) < 3:
                continue
            u, v, data = edge
            if u not in self._node:
                self._node_index[u] = self._id
                self._id += 1
                self._node[u] = self.node_attr_dict_factory()
                adj[u] = self.adjlist_inner_dict_factory()
            if v not in self._node:
                self._node_index[v] = self._id
                self._id += 1
            ...
            adj[v][u] = datadict
        self._adj = adj
        self.cache.pop("_lazy_edges", None)

    def _materialize_lazy_edge_arrays(self):
        lazy_arrays = self.cache.get("_lazy_edge_arrays")
        if lazy_arrays is None:
            return None
        try:
            src_idx, dst_idx, weights, weight_key = lazy_arrays
            idx_to_node = self.index2node
            edge_list = []
            for su, sv, w in zip(src_idx, dst_idx, weights):
                iu = int(su)
                iv = int(sv)
                if iu not in idx_to_node or iv not in idx_to_node:
                    continue
                edge_list.append(
                    (idx_to_node[iu], idx_to_node[iv], {weight_key: float(w)})
                )
        except Exception:
            edge_list = []
        self.cache["edges"] = edge_list
        self.cache["edge"] = edge_list
        self.cache["_lazy_edges"] = edge_list
        self.cache.pop("_lazy_edge_arrays", None)
        return edge_list
    ...
    def edges(self):
        ...
        if self.cache.get("_lazy_edges") != None:
            return self.cache["_lazy_edges"]
        if self.cache.get("_lazy_edge_arrays") != None:
            return self._materialize_lazy_edge_arrays()
        edge_lst = list()
        seen = set()
        for u in self._adj:
