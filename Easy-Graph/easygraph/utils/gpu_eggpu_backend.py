...
def _graph_context(G):
    return _prepare_graph_cache(G)
...
def pagerank(G, alpha=0.85, max_iter=200, eps=1e-6, weight=None):
    ...
    if cpp_hit is None:
        raise RuntimeError("native CUDA PageRank backend unavailable")
...
def minimum_spanning_tree_edges(G, weight="weight"):
    ...
    if cpp_hit is None:
        raise RuntimeError("native CUDA MST backend unavailable")
...
def clustering(G):
    ...
    if cpp_hit is None:
        raise RuntimeError("native CUDA clustering backend unavailable")
...
def connected_component_labels(G, directed):
    ...
    cpp_hit = _cpp_gpu_connected_component_labels(
        prepared,
        G,
        directed=bool(directed),
    )
    if cpp_hit is None:
        raise RuntimeError("native CUDA component backend unavailable")
