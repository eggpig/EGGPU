...
def minimum_spanning_tree(G, weight="weight", algorithm="kruskal", ignore_nan=False):
    ...
    """
    gpu_tree = _minimum_spanning_tree_gpu_runtime_dispatch(G, weight=weight)
    if gpu_tree is not None:
        return gpu_tree
...
def _populate_mst_tree_fast(T, G, edges, weight="weight"):
    """Populate an EasyGraph MST result without per-edge public API overhead."""
    try:
        node_index = getattr(T, "_node_index", None)
        inner_factory = T.adjlist_inner_dict_factory
        node_factory = T.node_attr_dict_factory
        edge_factory = T.edge_attr_dict_factory
        clear_cache = T._clear_cache
    except Exception:
        return False

    try:
        # MST results generally keep the original vertex set. Building the
        # node/adjacency dictionaries in bulk avoids two membership tests per
        # returned tree edge, which dominates large-graph E2E time after the
        # CUDA kernel has finished.
        nodes = list(G.nodes)
        T._adj = {n: inner_factory() for n in nodes}
        T._node = {n: node_factory() for n in nodes}
        ...
        for u, v, w in edges:
            datadict = {weight: w} if edge_factory is dict else edge_factory()
            if datadict.get(weight) != w:
                datadict[weight] = w
            adj[u][v] = datadict
            adj[v][u] = datadict
...
def _minimum_spanning_tree_gpu_runtime_dispatch(G, weight="weight"):
    ...
    try:
        ...
        if eggpu_backend.eggpu_backend_enabled():
            ...
            if not _populate_mst_tree_fast(T, G, edges, weight=weight):
                ...
                # Most real graph benchmarks have no isolated vertices; avoid
                # inserting every node twice through Python and only repair the
                # edgeless/isolated-node case when needed.
                if len(T) < len(G):
