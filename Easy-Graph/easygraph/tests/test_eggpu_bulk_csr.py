...
def test_logical_undirected_projection_preprocessor_and_loader(tmp_path):
    ...
    projection_manifest = (
        output_dir / "tiny-projection.logical-undirected.json"
    )
    projection = json.loads(
        projection_manifest.read_text(encoding="utf-8")
    )
    assert projection["unique_edge_count"] == 5
    assert projection["self_loops_removed"] == 1
    assert projection["duplicate_or_reciprocal_arcs_removed"] == 5
    assert set(projection["artifacts"]) == {
        "lower_V",
        "lower_E",
        "upper_V",
        "upper_E",
    ...
    assert all(
        ...
        for record in projection["artifacts"].values()
    )

    def load_i32(field):
        return np.fromfile(
            projection_manifest.parent / projection[field],
            dtype=np.int32,
        )

    assert np.array_equal(load_i32("lower_V_path"), [0, 2, 3, 4, 5, 5])
    assert np.array_equal(load_i32("lower_E_path"), [1, 2, 2, 3, 4])
    assert np.array_equal(load_i32("upper_V_path"), [0, 0, 1, 3, 4, 5])
    assert np.array_equal(load_i32("upper_E_path"), [0, 0, 1, 2, 3])
    assert np.array_equal(load_i32("degree_path"), [2, 2, 3, 2, 1])
    assert np.array_equal(load_i32("forward_V_path"), [0, 2, 3, 3, 4, 5])
    assert np.array_equal(load_i32("forward_E_path"), [1, 2, 2, 2, 3])

    base_only = eg.read_eggpu_csr(source_manifest)
    assert base_only.undirected_projection_info() == {"loaded": False}

    graph = eg.read_eggpu_csr(
        source_manifest,
        undirected_projection_path=projection_manifest,
    )
    info = graph.undirected_projection_info()
    assert info == {
        "loaded": True,
        "unique_edge_count": 5,
        "lower_V_size": 6,
    ...
    assert (
        estimate["host_total_with_projection_bytes"]
        == estimate["host_total_bytes"]
        + estimate["host_undirected_projection_bytes"]
    )


def test_undirected_projection_rejects_inconsistent_degree(tmp_path):
    source_manifest = _write_projection_source(tmp_path)
    projection_dir = tmp_path / "invalid-projection"
    ...
    projection_manifest = projection_dir / "invalid.json"
