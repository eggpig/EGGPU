...
ABLATION_TIMEOUT="${ABLATION_TIMEOUT:-100}"
BENCHMARK_REPEAT="${BENCHMARK_REPEAT:-5}"
MEMORY_REPEAT="${MEMORY_REPEAT:-1}"
EGGPU_WARMUP="${EGGPU_WARMUP:-2}"
...
CLOSENESS_LARGE_TIMEOUT="${CLOSENESS_LARGE_TIMEOUT:-100}"
...
MAIN_ID="full_eval_gpu${MAIN_GPU}_${RUN_TS}_repeat${BENCHMARK_REPEAT}_splitmem_exact_mst_strict_nxcg"
ABL_ID="ablation_gpu${ABL_GPU}_${RUN_TS}_repeat5_strengthened"
...
DEFAULT_GUNROCK_PATHS="${LEGACY_EVAL_ROOT}/gunrock_latest/build_cuda132_a100_migrated/bin:${LEGACY_EVAL_ROOT}/gunrock_legacy_master/build_lcc_result_cuda128_clean/bin"
EG_GUNROCK_BIN_PATHS="${EG_GUNROCK_BIN_PATHS:-${DEFAULT_GUNROCK_PATHS}}"
...
run_preflight() {
  env -u CFLAGS -u CPPFLAGS -u CXXFLAGS \
      ...
      EG_GUNROCK_BIN_PATHS="${EG_GUNROCK_BIN_PATHS}" \
...
run_ablation_one() {
  ...
  local function_count=1
  if [[ "${functions}" == "all" ]]; then
    function_count=16
  elif [[ "${functions}" == *,* ]]; then
    function_count="$(awk -F',' '{print NF}' <<< "${functions}")"
  fi
  local process_timeout="${ABLATION_PROCESS_TIMEOUT:-}"
  if [[ -z "${process_timeout}" ]]; then
    case "${experiment}" in
      workflow|return)
        # Workflow/return intentionally keep all functions in one process to
        # preserve shared graph state. Scale the outer safety bound so the
        # nominal allowance remains ABLATION_TIMEOUT per selected function.
        process_timeout=$(( ABLATION_TIMEOUT * function_count ))
        ;;
      *)
        process_timeout="${ABLATION_TIMEOUT}"
        ;;
    esac
  fi
  ...
  timeout "${process_timeout}" \
    env -u CFLAGS -u CPPFLAGS -u CXXFLAGS \
      ...
      "${COMMON_PY}" benchmarking/run_eggpu_ablations.py \
        ...
        --warmup "${warmup}" \
        --gpu "${ABL_GPU}" \
        --closeness-sources "${CLOSENESS_LARGE_SOURCES}" \
  ...
  if [[ "${rc}" -eq 124 ]]; then
    {
      echo "experiment,variant,dataset,graph_type,function,metric,value,unit,metric_family,measurement_scope,timer_kind,measurement_window,status,notes,repeat"
      echo "${experiment},${variant},${dataset_name},${graph_type},ALL,timeout_seconds,,s,time,ablation_component,perf_counter_wall,ablation_process,timeout,process timeout after ${process_timeout}s; nominal per-function allowance ${ABLATION_TIMEOUT}s,"
    } > "${out_path}"
    ABLATION_TIMEOUT_COUNT=$((ABLATION_TIMEOUT_COUNT + 1))
...
run_ablations() {
  ...
  local ORDER_VARIANTS=(full no_graph_context no_cpp_graph_cache)
  local REVERSE_ORDER="Hierarchy,Constraint,Efficiency,EffectiveSize,Closeness,BC,KCore,SSSP,BellmanFord,Dijkstra,BFS,SCC,WCC,LCC,MST,PageRank"
  local INTERLEAVED_ORDER="PageRank,LCC,EffectiveSize,BFS,WCC,Efficiency,Dijkstra,SCC,Constraint,BellmanFord,KCore,Hierarchy,SSSP,BC,Closeness,MST"
  local DATASET_TOTAL="${#SPECS[@]}"
  # Canonical order on all datasets plus reverse/interleaved orders on four
  # representative datasets for three graph-state variants.
  local TASK_TOTAL=$(( DATASET_TOTAL * ( ${#VARIANTS[@]} + 2 ) + 4 * 2 * ${#ORDER_VARIANTS[@]} ))
  local TASK_INDEX=0
  ...
  do
    ...
    do
      ...
      run_ablation_one \
        ...
        5 \
        2 \
        "${ABL_OUT}/workflow_${NAME}_canonical_${VARIANT}.csv" \
        --workflow-order-id canonical
    done

    case "${NAME}" in
      ca-GrQc|email-Enron|soc-Epinions1|wiki-Talk)
        for ORDER_ID in reverse interleaved
        do
          if [[ "${ORDER_ID}" == "reverse" ]]; then
            ORDER_FUNCTIONS="${REVERSE_ORDER}"
          else
            ORDER_FUNCTIONS="${INTERLEAVED_ORDER}"
          fi
          for VARIANT in "${ORDER_VARIANTS[@]}"
          do
            ablation_progress "${NAME}" workflow "${VARIANT}_${ORDER_ID}"
            run_ablation_one \
              workflow \
              "${VARIANT}" \
              "${PATH_IN}" \
              ...
              5 \
              2 \
              "${ABL_OUT}/workflow_${NAME}_${ORDER_ID}_${VARIANT}.csv" \
...
PY
  "${COMMON_PY}" benchmarking/summarize_ablation_system.py \
    --ablation-dir "${ABL_OUT}" \
    --out-dir "${ABL_OUT}" \
    --title "EGGPU Ablation System Summary"
  "${COMMON_PY}" - <<PY
from pathlib import Path
import json
...
protocol = {
    ...
    "measurement_mode": "timing_without_memory_sampling",
    "workflow": {
        "canonical_order_datasets": 17,
        "order_robustness_datasets": ["ca-GrQc", "email-Enron", "soc-Epinions1", "wiki-Talk"],
        "orders": ["canonical", "reverse", "interleaved"],
        "controlled_ratios": {
            "total_reusable_graph_state": "no_graph_context / full",
            "cpp_graph_cache": "no_cpp_graph_cache / full",
            "incremental_graph_context": "no_graph_context / no_cpp_graph_cache",
            "device_csr_cache": "no_device_csr_cache / full",
            "adaptive_policy": "no_adaptive_policy / full",
        },
    },
    "return_path": {
        "comparison": "deferred API-compatible result adapter versus built-in Python container constructed inside the paired function call",
        "pair_order": "alternated across repeats",
        "equivalence_gate": ["materialized element count", "numeric checksum"],
    },
    "layout": {
        "scope": "representation microbenchmark, not a full-system all-function format switch",
        "comparison": "equivalent COO and CSR adjacency slots with degree traversal and PageRank validation",
    },
    "claim_boundary": "Persistent workspace reuse has no independent off switch and is not reported as an independently ablated contribution.",
}
(out / "ablation_protocol.json").write_text(json.dumps(protocol, indent=2, sort_keys=True) + "\n")
PY
  if [[ "${ABLATION_TIMEOUT_COUNT}" -gt 0 ]]; then
    echo "Ablation incomplete: ${ABLATION_TIMEOUT_COUNT} task(s) reached the configured process timeout."
    return 2
  fi
}
...
run_main_audits() {
  ...
  echo "Best-observed sensitivity starting: OUT=${MAIN_OUT}/paper_internal_sensitivity, LOG=benchmarking/results/${MAIN_ID}.sensitivity.log"
  "${COMMON_PY}" benchmarking/generate_best_observed_sensitivity.py \
    "${MAIN_OUT}" \
    --near-miss-percent 5 \
    --expected-samples "${BENCHMARK_REPEAT}" \
    > "benchmarking/results/${MAIN_ID}.sensitivity.log" 2>&1
  SENSITIVITY_RC=$?
  if [[ "${SENSITIVITY_RC}" -ne 0 ]]; then
    echo "Best-observed sensitivity exit code: ${SENSITIVITY_RC}"
    echo "Best-observed sensitivity failed; see benchmarking/results/${MAIN_ID}.sensitivity.log"
    GROUP_RC=1
  else
    echo "Best-observed sensitivity written: OUT=${MAIN_OUT}/paper_internal_sensitivity"
  fi
...
if ! main_enabled; then
  if parallel_enabled; then
    echo "RUN_MAIN=FALSE ignores RUN_PARALLEL and runs ablation only."
  fi
  MAIN_RC=NOT_RUN
  AUDIT_GROUP_RC=0
  echo "Main eval skipped because RUN_MAIN=${RUN_MAIN}"
