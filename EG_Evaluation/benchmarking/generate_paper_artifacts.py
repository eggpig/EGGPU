...
def collect_return_ratios(abl_dir: Path):
    ...
    for path in abl_dir.glob("return_*.csv"):
        ...
        uses_new_schema = False
        with path.open(newline="") as fp:
            for row in csv.DictReader(fp):
                if row.get("status") != "ok":
                    continue
                metric = row.get("metric")
                if metric in {"call_return_seconds", "call_plus_forced_traversal_seconds"}:
                    uses_new_schema = True
                    if str(row.get("materialization_claim_valid", "")).lower() != "true":
                        continue
                    tmp[(row["dataset"], row["function"], metric)] = float(row["value"])
                elif metric in {"lazy_call_e2e", "eager_equivalent_e2e"}:
                    tmp[(row["dataset"], row["function"], row["metric"])] = float(row["value"])
        call_metric = "call_return_seconds" if uses_new_schema else "lazy_call_e2e"
        traversed_metric = (
            "call_plus_forced_traversal_seconds"
            if uses_new_schema
            else "eager_equivalent_e2e"
        )
        for dataset, function, metric in list(tmp.keys()):
            if metric != call_metric:
                continue
            call_seconds = tmp[(dataset, function, call_metric)]
            traversed_seconds = tmp.get((dataset, function, traversed_metric))
            if call_seconds and traversed_seconds:
                values[function].append(traversed_seconds / call_seconds)
    return values
...
def emit_return_ablation_figure(abl_dir: Path, out_dir: Path):
    ...
    ax.axhline(1.0, color="#73808C", linewidth=0.7, linestyle=(0, (3, 2)), zorder=1)
    ax.set_ylabel("Call plus traversal / call return")
...
def emit_ablation_summary_table(abl_dir: Path, out_dir: Path):
    ...
    if gm is not None:
        rows.append(("On-demand result view", "Call-plus-traversal/call ratio", gm, len(ret_values)))
...
def main():
    ...
    readme = [
        ...
        "- `ablation_return_path.pdf`: on-demand result-view traversal ablation.",
