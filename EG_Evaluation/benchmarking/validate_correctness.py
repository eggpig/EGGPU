...
_PAGERANK_TRANSITION_CACHE = {}
PAGERANK_ALPHA = 0.75
PAGERANK_MEAN_RESIDUAL_TOLERANCE = 1.0e-6
    ...
    with path.open() as f:
        for line in f:
            ...
            try:
                ...
                continue
            # The benchmark removes self-loop edges but retains every vertex
            # observed in the raw input.  Record endpoints before filtering so
            # vertices represented only by self-loops remain in the oracle.
            nodes.add(u)
            nodes.add(v)
            if u == v:
                continue
...
def _pagerank_transition(dataset, graph_type):
    cache_key = (dataset, graph_type)
    cached = _PAGERANK_TRANSITION_CACHE.get(cache_key)
    if cached is not None:
        return cached
    import numpy as np

    graph = _normalized_clean_edges(dataset)
    n = int(graph["n"])
    directed = graph_type == "directed"
    edges = graph["directed" if directed else "undirected"]
    src = np.fromiter((u for u, _ in edges), dtype=np.int32, count=len(edges))
    dst = np.fromiter((v for _, v in edges), dtype=np.int32, count=len(edges))
    out_degree = np.bincount(src, minlength=n).astype(np.float64)
    if not directed:
        out_degree += np.bincount(dst, minlength=n)
    value = (n, directed, src, dst, out_degree)
    _PAGERANK_TRANSITION_CACHE[cache_key] = value
    return value


def _pagerank_residual_check(row, parsed):
    """Validate PageRank by its defining equation, independent of solver stopping API."""

    detail, detail_err = _load_detail(parsed)
    if detail is None:
        return None, detail_err
    if detail.get("kind") != "vector":
        return None, f"PageRank residual validation needs vector detail, got {detail.get('kind')}"
    try:
        import numpy as np

        n, directed, src, dst, out_degree = _pagerank_transition(
            row.get("dataset", ""), row.get("graph_type", "")
        )
        values = np.asarray(detail["values"], dtype=np.float64)
        if values.shape != (n,):
            return False, f"PageRank detail shape mismatch: {values.shape} vs {(n,)}"
        if not np.isfinite(values).all():
            return False, "PageRank contains non-finite values"
        if float(values.min(initial=0.0)) < -1.0e-12:
            return False, "PageRank contains negative values"
        total = float(values.sum(dtype=np.float64))
        if not math.isclose(total, 1.0, rel_tol=1.0e-6, abs_tol=1.0e-8):
            return False, f"PageRank scores sum to {total:.12g}, expected 1"

        contribution = np.bincount(
            dst,
            weights=values[src] / out_degree[src],
            minlength=n,
        )
        if not directed:
            contribution += np.bincount(
                src,
                weights=values[dst] / out_degree[dst],
                minlength=n,
            )
        dangling_mass = float(values[out_degree == 0].sum(dtype=np.float64))
        expected = (
            PAGERANK_ALPHA * contribution
            + (PAGERANK_ALPHA * dangling_mass + (1.0 - PAGERANK_ALPHA)) / n
        )
        ...
        if mean_residual > PAGERANK_MEAN_RESIDUAL_TOLERANCE:
            return (
                False,
                f"PageRank mean fixed-point residual={mean_residual:.6g} exceeds "
                f"{PAGERANK_MEAN_RESIDUAL_TOLERANCE:.6g}; max={max_residual:.6g}",
            )
        return (
            True,
            f"PageRank fixed-point residual passes: mean={mean_residual:.6g}, "
            f"max={max_residual:.6g}, threshold={PAGERANK_MEAN_RESIDUAL_TOLERANCE:.6g}",
        )
    except Exception as exc:
        return None, f"PageRank residual validation unavailable: {exc}"
...
def _compare_details(function, parsed, ref_parsed):
    ...
    try:
        ...
        finite_a = np.isfinite(a)
        finite_b = np.isfinite(b)
        finite_mask_mismatch = int(np.count_nonzero(finite_a != finite_b))
        nan_mask_mismatch = int(np.count_nonzero(np.isnan(a) != np.isnan(b)))
        positive_inf_mask_mismatch = int(
            np.count_nonzero(np.isposinf(a) != np.isposinf(b))
        )
        negative_inf_mask_mismatch = int(
            np.count_nonzero(np.isneginf(a) != np.isneginf(b))
        )
        if (
            finite_mask_mismatch
            or nan_mask_mismatch
            or positive_inf_mask_mismatch
            or negative_inf_mask_mismatch
        ):
            return (
                False,
                "detail non-finite masks differ: "
                f"finite={finite_mask_mismatch}, nan={nan_mask_mismatch}, "
                f"posinf={positive_inf_mask_mismatch}, "
                f"neginf={negative_inf_mask_mismatch}",
            )
        jointly_finite = finite_a & finite_b
        maxdiff = (
            float(np.max(np.abs(a[jointly_finite] - b[jointly_finite])))
...
def _compare_directed_constraint_defined_domain(parsed, ref_parsed):
    """Compare directed Constraint where libraries differ on undefined vertices."""
    ...
    try:
        ...
        a = np.asarray(lhs["values"], dtype=np.float64)
        b = np.asarray(rhs["values"], dtype=np.float64)
        if a.shape != b.shape:
            return False, f"Constraint detail shape mismatch: {a.shape} vs {b.shape}"
        finite_a = np.isfinite(a)
        finite_b = np.isfinite(b)
        if np.any(~finite_a & finite_b):
            return False, "igraph is undefined where the aligned Constraint result is finite"
        jointly_finite = finite_a & finite_b
        if jointly_finite.any() and not np.allclose(
            a[jointly_finite], b[jointly_finite], rtol=1.0e-5, atol=1.0e-7
        ):
            maxdiff = float(np.max(np.abs(a[jointly_finite] - b[jointly_finite])))
            return False, f"directed Constraint differs on its defined domain, max_abs_diff={maxdiff:.6g}"
        convention_only = finite_a & ~finite_b
        if not convention_only.any():
            return True, "directed Constraint full vectors match"
        maxdiff = (
            float(np.max(np.abs(a[jointly_finite] - b[jointly_finite])))
            if jointly_finite.any()
            else 0.0
        ...
        return (
            True,
            f"defined-domain values match, max_abs_diff={maxdiff:.6g}; "
            f"undefined-vertex convention differs at {int(convention_only.sum())} entries",
        )
    except Exception as exc:
        return None, f"directed Constraint conditional comparison unavailable: {exc}"
...
def _function_checks(function):
    ...
    if function == "MST":
        # Gunrock's maintained CLI reports a float32 parallel reduction of the
        # selected-edge weights. The reduction order is nondeterministic, so
        # validate the total numerically rather than requiring integer bit
        # identity with CPU libraries that accumulate in a wider type.
        return (("weight", "float", 1e-4, 1e-3),), "pass"
    if function == "LCC":
        return (
...
def _validate_one(row, ref_row, ref_parsed):
    ...
    if (
        function == "PageRank"
        and baseline == "Gunrock"
        and "parameter contract is not aligned" in notes
    ):
        return (
            "semantic_mismatch",
            "Gunrock PageRank executable uses fixed alpha/tolerance values that differ from this run",
        )
    if function == "PageRank":
        parsed = _parse_correctness(row.get("correctness", ""))
        residual_ok, residual_msg = _pagerank_residual_check(row, parsed)
        if residual_ok is True:
            return "pass", residual_msg
        if residual_ok is False:
            return "fail", residual_msg
    if (
        function == "Constraint"
        and baseline == "igraph"
        and row.get("graph_type") == "directed"
    ):
        parsed = _parse_correctness(row.get("correctness", ""))
        conditional_ok, conditional_msg = _compare_directed_constraint_defined_domain(
            parsed, ref_parsed
        )
        if conditional_ok is True:
            return "partial_pass", conditional_msg
        if conditional_ok is False:
            return "fail", conditional_msg
        return "semantic_mismatch", conditional_msg
    parsed = _parse_correctness(row.get("correctness", ""))
    if not parsed:
...
def write_validation_outputs(out_dir, rows):
    ...
    if counts:
        ...
        for key in ("pass", "partial_pass", "sampled_pass", "weak_pass", "reference", "semantic_mismatch", "fail", "inconclusive", "inconclusive_self_reference"):
            if counts.get(key):
    ...
    md.append("PageRank vectors are validated against the PageRank fixed-point equation using the common damping factor and a predeclared mean absolute residual threshold, rather than requiring different solvers to expose identical stopping APIs.")
    md.append("Function-specific unsafe references are skipped when selecting an oracle: BC does not use nx-cugraph because its large-graph values differ from NetworkX/igraph/EasyGraph semantics in existing validation rows.")
    ...
    md.append("Directed igraph Constraint rows are marked `partial_pass` when all jointly defined values match but the treatment of zero-outdegree vertices differs; this status is documented but excluded from strict all-node timing comparisons.")
    ...
    md.append("PageRank falls back to summary-only `weak_pass` only when no full vector detail is available.")
