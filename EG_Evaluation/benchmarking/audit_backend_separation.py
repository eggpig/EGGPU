...
def _extract_function_from_log(path: Path, baseline: str) -> str:
    ...
    if name.startswith(prefix) and name.endswith(".log"):
        function = name[len(prefix) : -4]
        return re.sub(r"_r\d+$", "", function)
    return ""
...
def audit_logs(result_dir: Path) -> tuple[list[dict], Counter]:
    ...
    if not logs_dir.exists():
        split_logs = result_dir / "measurement_passes" / "timing" / "logs"
        if split_logs.exists():
            logs_dir = split_logs
    rows: list[dict] = []
...
def write_markdown(out_dir: Path, result_dir: Path, log_rows: list[dict], result_rows: list[dict]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Expected Contract",
            "",
            "- EGGPU: `mode=gpu`, `EASYGRAPH_ENABLE_GPU=TRUE`, `easygraph_warmup=2`.",
            "- easygraph-cpp: `mode=cpp`, `EASYGRAPH_ENABLE_GPU=FALSE`, `easygraph_warmup=0`.",
            "- easygraph-cpu: `mode=cpu`, `EASYGRAPH_ENABLE_GPU=FALSE`, `easygraph_warmup=0`.",
