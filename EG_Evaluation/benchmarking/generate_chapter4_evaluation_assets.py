...
FAMILY_COLOR = {
    "Centrality": "#8775C9",
    "Connectivity": "#5E9BC5",
    "Paths & Spanning Trees": "#71B18F",
    "Structural Holes": "#D28D52",
}
FAMILY_TINT = {
    "Centrality": "#EEEAFB",
    "Connectivity": "#E8F2F9",
    "Paths & Spanning Trees": "#E9F5EE",
...
def write_dataset_table(source_dir: Path, output_dir: Path) -> None:
    ...
    for row in data.itertuples(index=False):
        prefix = r"\rowcolor{ScaleAnchorTint}" if row.role != "cross-library" else ""
        lines.append(
            prefix
            + "{} & {} & {} & {:.2f} & {} & {} & {} \\\\".format(
    ...
    lines.extend(
        [
            ...
            r"\end{tabular}",
            r"\vspace{-2pt}",
            r"\begin{minipage}{0.98\textwidth}\footnotesize Scale anchors are shaded: com-Orkut represents the 100-million-edge regime and GAP-twitter the billion-edge regime.\end{minipage}",
            r"\end{table*}",
            "",
        ]
    )
...
def write_settings_table(output_dir: Path) -> None:
    lines = [
        ...
        r"Systems & EGGPU/EasyGraph 1.6, EGGPU-2024 at its archived EasyGraph revision, NetworkX 3.4.2, igraph 1.0.0, nx-cugraph 26.2.0, and Gunrock v2.2.0. Legacy Gunrock v1.x is retained for provenance but not substituted for missing current algorithms. \\",
        ...
        r"Timing & Five independent timing processes per cell; 100-s per-call timeout. Each EGGPU process performs two untimed function calls before its measured reused-state call; competitors are not warmed. \\",
        r"Estimator & Arithmetic mean of five independent timing processes $\pm$ sample standard deviation for every system. \\",
...
def write_representative_table(source_dir: Path, output_dir: Path) -> pd.DataFrame:
    ...
    for dataset, function, family, alias in REPRESENTATIVE_PAIRS:
        color_name = {
            "Centrality": "CentralityTint",
            "Connectivity": "ConnectivityTint",
            "Path & Spanning": "PathsTint",
            "Structural Holes": "StructuralTint",
        }[family]
        header_cells.append(
            rf"\cellcolor{{{color_name}}}\shortstack{{{latex_escape(alias)}\\{latex_escape(function)}}}"
        )
    lines = [
        "% Generated by generate_chapter4_evaluation_assets.py",
        r"\begin{table*}[t]",
        r"\centering",
        r"\caption{Representative end-to-end latency in milliseconds across all four function families. Columns span six distinct directed and undirected graphs and favor workloads with broad correctness-validated baseline support. Bold green values are fastest and orange values are second fastest. Every cell reports the arithmetic mean of five independent timing processes $\pm$ sample standard deviation.}",
...
def write_first_call_and_ablation_table(
    ...
    family_tint = {
        "Centrality": "CentralityTint",
        "Connectivity": "ConnectivityTint",
        "Paths & Spanning Trees": "PathsTint",
        "Structural Holes": "StructuralTint",
    }
    ablation_label = {
    ...
    lines = [
        ...
        r"\begin{table*}[t]",
        r"\centering",
        r"\caption{First-call cost and controlled ablations. (a) Geometric-mean E2E latency for a first call and a later call that reuses compatible graph state. (b) Cost increase after disabling or replacing one mechanism; larger values indicate greater degradation.}",
    ...
    for family in FAMILIES:
        ...
        lines.append(
            rf"\rowcolor{{{family_tint[family]}}}{latex_escape(FAMILY_DISPLAY[family])} & "
...
def scaling_figure(source_dir: Path, output_dir: Path) -> None:
    ...
    for panel_index, (axis, function) in enumerate(zip(axes, SCALING_FUNCTIONS)):
        ...
        axis.set_ylabel("End-to-end time (s)")
...
def workflow_figure(workflow_dir: Path, output_dir: Path) -> None:
    ...
    axis.set_ylabel("Cumulative E2E time (s)")
...
def category_overview_figure(
    ...
    if timing_ledger is None:
        ...
        metric_systems = {
            "Graph construction": tuple(
                system for system in BASELINE_ROWS if system != "Gunrock"
            ),
            "Processing time": tuple(
                system for system in BASELINE_ROWS if system != "nx-cugraph"
            ),
            "End-to-end": tuple(
                system for system in BASELINE_ROWS if system != "Gunrock"
            ),
    ...
    if "timing_boundary" not in data:
        data["timing_boundary"] = data.apply(
            lambda row: (
                ...
                if row["metric"] == "Graph construction"
                    ...
                    and row["baseline"] in {"EGGPU", "Gunrock"}
                    else (
                        "in-memory algorithm wall-time surrogate"
                        if row["metric"] == "Processing time"
                        else "public in-process call"
                    )
                )
            ),
            axis=1,
    ...
    fig, axes = plt.subplots(1, 3, figsize=(7.2, 2.48), sharey=True)
    ...
    for axis, (metric_aliases, title) in zip(axes, metrics):
        ...
        for index, baseline in enumerate(baseline_order):
            ...
            axis.scatter(
                ...
                y + offset,
                marker=SYSTEM_MARKER[baseline],
                s=44 if baseline == "EGGPU" else (31 if baseline == "EGGPU-2024" else 27),
                color=SYSTEM_COLOR[baseline],
    ...
    fig.subplots_adjust(top=0.70, bottom=0.23, left=0.18, right=0.995, wspace=0.17)
...
def main() -> None:
    ...
    manifest = {
        ...
        "timing_estimators": {
            "all_systems": "arithmetic mean of five plus sample standard deviation",
        },
