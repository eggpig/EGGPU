...
import run_eggpu_ablations as ablation_runner


DEFAULT_FUNCTIONS = ablation_runner.DEFAULT_FUNCTIONS
...
def run_dataset(args, size, graph_type, name, edge_path, functions):
    ...
    rows = ablation_runner.run_workflow(workflow_args)
    for row in rows:
...
def main():
    ...
    ap.add_argument("--closeness-sources", type=int, default=16)
    ...
    ablation_runner.MEASUREMENT_MODE = "timing"
