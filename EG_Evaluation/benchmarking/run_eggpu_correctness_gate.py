...
def _add_row(rows, size, graph_type, dataset, result, log_path, rc, timeout):
    ...
    descriptor = describe_metric(result.get("metric", "e2e"), result.get("backend", ""))
    rows.append(
        {
            "dataset_size": size,
            ...
            "value": "" if seconds is None else float(seconds),
            **descriptor,
            "status": status,
            "log": str(log_path),
...
def _run_one(
    ...
    if not results:
        for metric in ("build", "e2e", "kernel"):
            descriptor = describe_metric(metric, backend)
            rows.append(
                {
                    "dataset_size": size,
                    "graph_type": graph_type,
...
def _eggpu_gate_issues(rows, validation, datasets, functions):
    expected = {(dataset[2], function) for dataset in datasets for function in functions}
    successful_e2e = {
    ...
    validated = {
        ...
        if row.get("baseline") == "EGGPU"
        and row.get("validation_status") in {"pass", "sampled_pass"}
    }

    issues = []
    for dataset, function in sorted(expected - successful_e2e):
        issues.append(f"missing successful EGGPU e2e row: {dataset}/{function}")
    for dataset, function in sorted(expected - validated):
        issues.append(f"missing passing EGGPU validation: {dataset}/{function}")
    return issues
...
def main():
    ...
    for size, graph_type, dataset, edge_path in datasets:
        ...
        for function in functions:
            ...
            # Closeness also uses the exact C++ implementation so Python CPU
            # timeout does not leave a false validation gap on medium graphs.
            reference_backends = (
                ("easygraph-cpu", "easygraph-cpp")
                if function == "Closeness"
                else ("easygraph-cpu",)
            )
            for backend in (*reference_backends, "EGGPU"):
                _run_one(
                    rows,
                    base_env,
                    out_dir,
    ...
    gate_issues = _eggpu_gate_issues(rows, validation, datasets, functions)
    ...
    if gate_issues:
        for issue in gate_issues:
            print(f"[correctness-gate] {issue}")
        raise SystemExit(2)
