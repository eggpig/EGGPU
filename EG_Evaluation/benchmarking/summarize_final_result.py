...
MODE_RE = re.compile(
    ...
    r"mode=(?P<mode>\S+)\s+"
    r"EASYGRAPH_ENABLE_GPU=(?P<enable>\S*)"
)
...
def summarize_metric(result_dir: Path, metric: str, filter_name: str, dataset_predicate) -> tuple[dict, pd.DataFrame]:
    ...
    for dataset, function in pairs:
        ...
        if eggpu_ok.empty:
            if competitors.empty:
                ...
                best_row = competitors.sort_values("seconds_num").iloc[0]
            ...
            rows.append(
                {
                    ...
                    "is_sota": False,
                    "is_near_miss": False,
                    "has_comparator": not competitors.empty,
                    "coverage_only": False,
            ...
            rows.append(
                {
                    ...
                    "best_baseline": "",
                    "best_seconds": None,
                    "ratio_to_best": math.nan,
                    "sota_gap_pct": math.nan,
                    "is_sota": False,
                    "is_near_miss": False,
                    "has_comparator": False,
                    "coverage_only": True,
                    "notes": "validated EGGPU result; no aligned validated comparator",
                }
            )
            continue
    ...
    summary = {
        ...
        "all_eggpu_pairs": all_eggpu_pairs,
...
def category_aggregate_summary(details: pd.DataFrame) -> pd.DataFrame:
    ...
    for (metric, filter_name, category), sub in details.groupby(["metric", "filter", "category"], sort=True):
        if "has_comparator" in sub.columns:
            sub = sub[sub["has_comparator"].fillna(False).astype(bool)]
        if sub.empty:
            continue
...
def write_markdown(
    ...
    lines = [
        ...
        f"- Full comparable-pair E2E SOTA coverage: {goal['e2e_full_pair_sota_pct']:.1f}%",
        f"- Full comparable-pair kernel SOTA coverage: {goal['kernel_full_pair_sota_pct']:.1f}%",
        "",
        "## Correctness Gate",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "Pairs with no aligned validated comparator are reported as unique coverage and are not counted as SOTA wins.",
            "",
            "| Metric | Filter | SOTA Pairs | Comparable Pairs | Unique Coverage | Coverage |",
            "|---|---|---:|---:|---:|---:|",
        ]
    )
    for summary in summaries:
        lines.append(
            ...
            f"{summary['total_pairs']} | {summary['coverage_only_pairs']} | {summary['sota_pct']:.1f}% |"
        )
        lines.append("")
        ...
        [
            {
                ...
                "all_eggpu_pairs": summary["all_eggpu_pairs"],
                "coverage_only_pairs": summary["coverage_only_pairs"],
